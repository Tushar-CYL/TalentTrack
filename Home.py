...
def streamlit_navbar():
    ...
    with col2:
        if st.button("💼 Find Jobs"):
            st.experimental_set_query_params(page="find-jobs")
    with col3:
        if st.button("📊 Predictions"):
            st.experimental_set_query_params(page="predictions")
    with col4:
        if st.button("👥 New User"):
            st.experimental_set_query_params(page="new-user")
    with col5:
        if st.button("📝 Resume Score"):
            st.experimental_set_query_params(page="resume-score")
    with col6:
...
def main():
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        st.image("./assets/Career_Isometric.png", use_container_width=True)
    with col2:
        st.image("./assets/career.png", use_container_width=True)
    with col3:
        st.image("./assets/Career_Outline.png", use_container_width=True)
