...
def extract_text_from_pdf(file_path):
    ...
    text = ""
    pdf_document = fitz.open("pdf", file_path)
...
def filter_relevant_text(text):
    ...
    # Example of filtering logic; adjust as needed based on specific resume formats.
    lines = text.split('\n')
    relevant_lines = []
    capture = False

    for line in lines:
        # Start capturing from specific keywords
        if re.search(r"(experience|education|skills|projects|achievements|certifications)", line, re.IGNORECASE):
            capture = True
        # Stop capturing on keywords that suggest a cover letter or contact info
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            # Mapping of prediction categories
            category_mapping = {
                0: "Advocate",
                1: "Arts",
